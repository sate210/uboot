...
fastboot_ptentry ptable_default[] =
{
	...
	{
		.name     = "recovery",
		.start    = BOOTLOADER_SIZE,
		.length   = RECOVERY_SIZE,
		.flags    = 0
	},
	{
		.name     = "kernel",
		.start    = BOOTLOADER_SIZE+RECOVERY_SIZE,
		.length   = KERNEL_SIZE,
		.flags    = 0
	},
	{
		.name     = "ramdisk",
		.start    = BOOTLOADER_SIZE+RECOVERY_SIZE+KERNEL_SIZE,
		.length   = RAMDISK_SIZE,
		.flags    = FASTBOOT_PTENTRY_FLAGS_WRITE_YAFFS
	},
	{
		.name     = "system",
		.start    = BOOTLOADER_SIZE+RECOVERY_SIZE+KERNEL_SIZE+RAMDISK_SIZE,
		.length   = SYSTEM_SIZE,
		.flags    = FASTBOOT_PTENTRY_FLAGS_WRITE_YAFFS
	},
	{
		.name     = "cache",
		.start    =  BOOTLOADER_SIZE+RECOVERY_SIZE+KERNEL_SIZE+RAMDISK_SIZE+SYSTEM_SIZE,
		.length   = CACHE_SIZE,
		.flags    = FASTBOOT_PTENTRY_FLAGS_WRITE_YAFFS
	},
	{
		.name     = "userdata",
		.start    =  BOOTLOADER_SIZE+RECOVERY_SIZE+KERNEL_SIZE+RAMDISK_SIZE+SYSTEM_SIZE+CACHE_SIZE,
		.length   = USERDATA_SIZE,
		.flags    = FASTBOOT_PTENTRY_FLAGS_WRITE_YAFFS
	}
};
